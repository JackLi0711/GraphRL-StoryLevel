...
from Validation import sections_tw
from Validation import structure


def generate(args):
    target_dir, structure_dir, section_country = args
    print(f"Generating {target_dir}/{structure_dir} strucutre graph")
    
    # set section type
    if section_country == "USA": pass
    ...
    if elem_file_size < 10:
        print(f"Structure {folder_name} has no elem_file, skip it.")
        raise ValueError("stoppppppppp")
        return
    ...
    try:
        ...
        print(f"Structure has some bug, skip it.")  # index: {structure_index}
        return
    ...
    for node_name in node_dict.keys():
        RigidZones[node_name] = structure.RigidZone(node_name=node_name, timestep=timestep)
    ...
    torch.save(data, join(folder_name, 'structure_graph_NodeAsNode.pt'))
...
def generate_graph_NodeAsNode(target_dir):
    processes = 6
    ...
    for structure_dir in os.listdir(target_dir):
        ...
        args = (target_dir, structure_dir, "Taiwan")
        p_pool.append(args)
