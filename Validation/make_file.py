...
def set_Rayleigh_coeff(root, target_dir):
    target_dir = os.path.join(root, target_dir)
    for ground_motion_name in os.listdir(target_dir):
        folder_name = os.path.join(target_dir, ground_motion_name)
        print(f"Setting rayleigh for: {folder_name}")
