...
from Validation import normalization    
...
class PlasticHingeClassifier(nn.Module):
    def __init__(self, yield_factor=0.90, specific_location=False, local_axis='Z', neglect_beam_My=True, norm_dict=None):
        super(PlasticHingeClassifier, self).__init__()
        self.specific_location = specific_location
        self.local_axis = local_axis
        self.neglect_beam_My = neglect_beam_My
        self.norm_dict = norm_dict

        if self.local_axis == 'Z':  # MomentZ (strong-axis)
            # X feature
            self.Myield_yn_index = 28
            self.Myield_yp_index = 30
            self.section_info_dim = 2
            self.Myield_start_index = 24
        ...
        elif self.local_axis == 'Y':  # MomentY (weak-axis)
            # X feature
            self.Myield_yn_index = 28
            self.Myield_yp_index = 30
            self.section_info_dim = 2
            self.Myield_start_index = 28 if self.neglect_beam_My else 24
            self.Myield_end_index = 28 + (2 - 1) * self.section_info_dim + 1 if self.neglect_beam_My else 24 + (6 - 1) * self.section_info_dim + 1
            # Y target
            self.output_yn_index = 8
            self.output_yp_index = 9
            self.output_start_index = 8 if self.neglect_beam_My else 6
            self.output_end_index = 10 if self.neglect_beam_My else 12

        else:
            raise ValueError("local_axis should be either 'Z' or 'Y'")
    ...
    def forward(self, x, node_out, node_y):
        if self.specific_location:
            ...
            # node_out, node_y: [number of sample nodes in a batch, timesteps, output dimension]
