...
class OptionCriticAgent(Agent):
    def __init__(self, 
        ...
        self.option_critic_network = model.OptionCritic(**actor_critic_kwargs).to(self.device)
        ...
        params_critic = list(self.option_critic_network.option_critic.parameters())
    ...
    def choose_option(self, state: torch.Tensor, greedy=False):
        with torch.no_grad():
            _, option_values = self.option_critic_network.forward(state)  # (num_options,)
            print(f"\t{option_values = }")

        epsilon = 0 if greedy else self._epsilon_decay_schedule(self._number_episodes)
        option_idx = self._random_state.randint(len(option_values)) if self._random_state.random() < epsilon else torch.argmax(option_values).item()
        option_value = option_values[option_idx]
        print(f"\t{epsilon = }, {option_idx = }")

        return option_idx, option_value.item()
    ...
    def choose_action(self, state: torch.Tensor, infeasible_actions: List[int], option_idx: int, greedy=False):
        ...
        with torch.no_grad():
            logits_all, option_values = self.option_critic_network.forward(state)  # (num_options, num_actions), (num_options,)
    ...
    def step(self):
        ...
        if all(batch.shape == batched_states[0].shape for batch in batched_states):
            ...
            batched_logits_all, batched_option_values = self.option_critic_network.forward(batched_states)  # (batch_size, num_options, num_actions), (batch_size, num_options)
            print(f"\t\t{batched_logits_all.shape = }, {batched_option_values.shape = }")

            batched_masks = infeasible_masks.unsqueeze(1).repeat(1, self.num_options, 1)  # (batch_size, num_options, num_actions)
            masked_batched_logits = batched_logits_all.masked_fill(batched_masks, float('-inf'))  # (batch_size, num_options, num_actions)
            batched_probs_all = nn.functional.softmax(masked_batched_logits, dim=-1)  # (batch_size, num_options, num_actions)

            all_gjsds = []
            all_probs = []
            all_option_values = []
            for probs, option_values, option_idx in zip(batched_probs_all, batched_option_values, batched_options):
                gjsd = self.compute_gjsd(probs)
                all_gjsds.append(gjsd)
                all_probs.append(probs[option_idx])  # (num_actions,)
                all_option_values.append(option_values[option_idx])  # scalar
            ...
            values = torch.stack(all_option_values)  # (batch_size,)
            ...
            for state, mask, option_idx, action in zip(batched_states, infeasible_masks, batched_options, actions):
                logits_all, option_values = self.option_critic_network.forward(state.to(self.device)) # (num_options, num_actions), (num_options,)
            ...
            values = torch.stack(all_option_values).to(self.device)  # (batch_size,)
            print(f"\t\t{values.shape = }")
        ...
        print(f"\t[loss coeff] actor: {self.actor_loss_coef:.4f}, entropy: {entropy_weight:.4f}, critic: {self.critic_loss_coef:.4f}, GJSD: {self.gjsd_loss_coef:.4f}")
        print(f"\t[loss value] actor: {actor_loss:.4f}, entropy: {mean_entropy:.4f}, critic: {critic_loss:.4f}, GJSD: {mean_gjsd:.4f}")
        ...
        critic_grad_norm = torch.tensor([p.grad.norm().item() ** 2 for p in self.option_critic_network.option_critic.parameters() if p.grad is not None]).mean().sqrt()
        ...
        if self.max_grad_norm is not None:
            ...
            nn.utils.clip_grad_norm_(self.option_critic_network.option_critic.parameters(), self.max_grad_norm)
...
def _flat_training(agent: OptionCriticAgent,
    ...
    while not done:
        ...
        option_idx, option_value = agent.choose_option(state)
        ...
        transition = {
            ...
            "option_rollout_length": 1 if not done else 0, 
        }
...
def _flat_testing(agent: OptionCriticAgent,
    ...
    while not done:
        ...
        with torch.no_grad():
            graph = graph.to(agent.device)
            state = agent.gnn.forward(graph.x, graph.edge_index, graph.edge_attr, None, structure.aux["story_batch"].to(agent.device), None)
        option_idx, option_value = agent.choose_option(state, greedy=True)
...
def _hierarchical_training(agent: OptionCriticAgent,
    ...
    while not done:
        ...
        with torch.no_grad():
            graph = graph.to(agent.device)
            state0 = agent.gnn.forward(graph.x, graph.edge_index, graph.edge_attr, None, structure.aux["story_batch"].to(agent.device), None)
        option_idx, option_value = agent.choose_option(state0)
        ...
        # expensive update (with analysis)
        ...
        transition = {
            ...
            "option_rollout_length": option_rollout_length if not done else 0, 
        }
        agent.storage.store(transition)
        graph = structure.graph.clone()
...
def _hierarchical_testing(agent: OptionCriticAgent,
    ...
    while not done:
        ...
        with torch.no_grad():
            graph = graph.to(agent.device)
            state0 = agent.gnn.forward(graph.x, graph.edge_index, graph.edge_attr, None, structure.aux["story_batch"].to(agent.device), None)
        option_idx, option_value = agent.choose_option(state0, greedy=True)
        option_rollout_length = 0
...
def _update_and_record(agent: OptionCriticAgent, rec: Record) -> None:
    ...
    params_critic_before = [p.clone().detach() for p in agent.option_critic_network.option_critic.parameters()]
    ...
    for epoch in range(agent.optimization_epoch):
        ...
        agent.logger.info(f"Optim epoch: {epoch+1:2d}, Loss - Actor: {info['actor_loss']:.4f}, Entropy: {info['entropy_loss']:.4f}, Critic: {info['critic_loss']:.4f}, GJSD: {info['gjsd_loss']:.4f}")
        agent.logger.info(f"Grad norm - GNN: {info['gnn_grad_norm']:.4f}, Actor: {info['actor_grad_norm']:.4f}, Critic: {info['critic_grad_norm']:.4f}")
    t_end = time.time()
    ...
    params_critic_after = [p.clone().detach() for p in agent.option_critic_network.option_critic.parameters()]
    ...
    agent.logger.info(f"Param change - GNN: {gnn_change:.6f}, Actor: {actor_change:.6f}, Critic: {critic_change:.6f}")
...
def _record_testing_rollout(agent: OptionCriticAgent, rec: Record) -> None:
    if not agent.use_gae:
        ...
        advantages = []
        gae = 0
        values = agent.storage.value + [0]  # add dummy value for convenience in calculation
        for i in reversed(range(len(agent.storage.reward))):
