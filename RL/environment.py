...
class Environment:
    ...
    def _init_testing_structure(self, initial_design=None):
        ...
        if self.structure_shape in ["fixed", "small_random"]:
            ...
            # FIXED: Testing structure for random training
            # Should match the actual testing target (6F 4x4)
            x_span_num = 4  # Changed from 3 to 4
            z_span_num = 4  # Changed from 3 to 4
            x_span_len = 6000
            z_span_len = 8000
            x_span_lens = [x_span_len for i in range(x_span_num)]
            z_span_lens = [z_span_len for i in range(z_span_num)]
            story_num = 6  # Changed from 5 to 6
            story_height = 3200
