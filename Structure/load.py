...
class NodalLoad:
    def __init__(self, 
                 ...
                 structure: Structure, 
                 load_case: LoadCase, 
                 horizontal_earthquake_loads: List[Dict[str, float]], 
                 horizontal_scale_factors: List[Dict[str, float]],
                 vertical_earthquake_load: float,
                 Fus: List[float], 
                 Fuv: float,
                 drift_case=False, 
        ...
        earthquake_load_xdir, earthquake_load_zdir = horizontal_earthquake_loads
        scale_factor_xdir, scale_factor_zdir = horizontal_scale_factors
        Fu_xdir, Fu_zdir = Fus
        if (self.E_x_n or self.E_x_p):
            horizontal_earthquake_load = earthquake_load_xdir
            horizontal_scale_factor = scale_factor_xdir
            Fu = Fu_xdir
            direction = 1
        else: 
            horizontal_earthquake_load = earthquake_load_zdir
            horizontal_scale_factor = scale_factor_zdir
            Fu = Fu_zdir
            direction = 2
        
        if drift_case:
            earthquake_load_type = "V_star"
        else:
            earthquake_load_index = np.array(list(horizontal_earthquake_load.values())).argmax()
            earthquake_load_type = list(horizontal_earthquake_load.keys())[earthquake_load_index]

        self.earthquake_load_type = earthquake_load_type
        self.horizontal_earthquake_load = horizontal_earthquake_load[self.earthquake_load_type]
        self.horizontal_scale_factor = horizontal_scale_factor[self.earthquake_load_type]
        self.vertical_earthquake_load = vertical_earthquake_load
        self.Fu = Fu
        self.Fuv = Fuv
    ...
    def __str__(self):
        info = f"{self.load_name}, D: {self.D:.1f}, L: {self.L:.1f}, E_x_n: {self.E_x_n:.1f}, E_x_p: {self.E_x_p:.1f}, E_z_n: {self.E_z_n:.1f}, E_z_p: {self.E_z_p:.1f}, E_y: {self.E_y:.1f}, "
        info += f"if_drift_case: {self.drift_case}, if_col_strength_case: {self.col_strength_case}, "
        info += f"\n{self.earthquake_load_type}, horizontal_earthquake_load: {self.horizontal_earthquake_load:.3f}, vertical_earthquake_load: {self.vertical_earthquake_load:.3f}, "
        return info
    ...
    def _distribute_vertical_load(self, structure: Structure) -> np.ndarray:
        ...
        # vertical_earthquake_force = self.E_y * self.horizontal_earthquake_load * np.array(list(structure.node_Ey_ratio_dict.values()))
        vertical_earthquake_force = self.E_y * self.vertical_earthquake_load * np.array(list(structure.node_Ey_ratio_dict.values()))
    ...
    def _distribute_horizontal_load(self, structure: Structure) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.E_x_n > 0:
            horizontal_x_load = -1 * self.E_x_n * self.horizontal_earthquake_load * np.array(list(structure.node_Exn_ratio_dict.values()))
        elif self.E_x_p > 0:
            horizontal_x_load = +1 * self.E_x_p * self.horizontal_earthquake_load * np.array(list(structure.node_Exp_ratio_dict.values()))
        elif self.E_z_n > 0:
            horizontal_z_load = -1 * self.E_z_n * self.horizontal_earthquake_load * np.array(list(structure.node_Ezn_ratio_dict.values()))
        elif self.E_z_p > 0:
            horizontal_z_load = +1 * self.E_z_p * self.horizontal_earthquake_load * np.array(list(structure.node_Ezp_ratio_dict.values()))
