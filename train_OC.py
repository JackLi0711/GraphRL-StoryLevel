...
from RL import agent_OC, environment, record
from Visualization import plot_OC, plot_PPO
...
def main(args):
	...
	rec = record.Record(additional_info={
		...
		"entropies": {"train": [], "test": []},  # train/test: (episode_num, timestep_num)
		...
		"losses": {"actor": [], "entropy": [], "critic": [], "gjsd": []},  # actor/entropy/critic/gjsd: (episode_num, optimization_epoch)
		"grad_norms": {"gnn": [], "actor": [], "critic": []},  # gnn/actor/critic: (episode_num, optimization_epoch)
		"param_changes": {"gnn": [], "actor": [], "critic": []},  # gnn/actor/critic: (episode_num)
        # Option-Critic
		"option_indices": {"train": [], "test": []},  # train/test: (episode_num, timestep_num)
        "gen_js_divergences": {"train": [], "test": []}  # train/test: (episode_num, timestep_num)
    })
    ...
    # Visualization
	plot_OC.plot_return(rec.training_record["score"], rec.testing_record["score"], args.option_num, args.ckpt_dir)
	plot_OC.plot_fail_name(rec.training_record["fail_name"], rec.testing_record["fail_name"], rec.option_indices["train"], rec.option_indices["test"], args.option_num, args.ckpt_dir)
	plot_OC.plot_fail_reason(rec.training_record["fail_reason"], rec.testing_record["fail_reason"], rec.option_indices["train"], rec.option_indices["test"], args.option_num, args.ckpt_dir)
	plot_OC.plot_testing_behavior(rec, env, args.option_num, args.ckpt_dir)
	
	plot_OC.plot_advantage(rec.advantages, args.option_num, args.ckpt_dir)
	plot_OC.plot_entropy(rec.entropies, args.option_num, args.ckpt_dir)
