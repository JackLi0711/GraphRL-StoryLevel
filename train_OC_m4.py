...
from RL import agent_OC_m4, environment, record
from Visualization import plot, plot_PPO, plot_OC_m3, plot_OC_m4
...
def parse_args() -> Namespace:
	...
	parser.add_argument("--comment", type=list[str], default=[
		...
		"advantage estimation: OC-style (vanilla)", 
		"initialization for network parameters - weight: orthogonal, bias: constant (A: 0, C: 0, B: 0)", 
		"intra-option policy: overall actor network"
	])
	...
	parser.add_argument("--suffix", type=str, 
		default="OC_LR5eN4_LossCoef_OCA10_C001_B1H0Reg1to001Tar05_GJSD0_OptNum2_OptimEpo1_SyncFreq10_LinDecay01_Episode1000"
	)
	...
	parser.add_argument("--actor_loss_type", type=str, default="OC", choices=["PPO", "OC"])
	parser.add_argument("--actor_loss_coef", type=float, default=10.0)
	parser.add_argument("--critic_loss_coef", type=float, default=0.01)
	parser.add_argument("--beta_loss_coef", type=float, default=1.0)
	...
	parser.add_argument("--termination_reg", type=float, default=0.0)
	parser.add_argument("--if_hierarchical", action="store_true", default=False, help="whether to use hierarchical option-critic or flat style")
	parser.add_argument("--sync_target_freq", type=int, default=10, help="synchronize target network with online network every sync_target_freq epochs")
...
def main(args):
	...
	agent_model = agent_OC_m4.OptionCriticAgent(**_agent_kwargs)
	...
	rec = record.Record(additional_info={
		# Actor-Critic
		"returns": {"train": [], "test": []},  # train: (episode_num, optim_epoch, timestep_num), test: (episode_num, timestep_num)
		"advantages": {"train": [], "test": []},  # train: (episode_num, optim_epoch, timestep_num), test: (episode_num, timestep_num)
		"beta_advantages": {"train": [], "test": []},  # train: (episode_num, optim_epoch, timestep_num), test: (episode_num, timestep_num)
        "entropies": {"train": [], "test": []},  # (episode_num, timestep_num)
		"pred_values": [],  # (episode_num, optim_epoch, timestep_num)
		"kl_divergences": [],  # (episode_num, optim_epoch)
		"ratios": [],  # (episode_num, optim_epoch, timestep_num)
		"losses": {"actor": [], "entropy": [], "critic": [], "beta": [], "gjsd": []},  # (episode_num, optim_epoch)
		"grad_norms": {"gnn": [], "actor": [], "critic": [], "beta": []},  # (episode_num, optim_epoch)
		"param_changes": {"gnn": [], "actor": [], "critic": [], "beta": []},  # (episode_num)
		# Option-Critic
		...
		"all_betas": {"train": [], "test": []}, # train: (episode_num, optim_epoch, timestep_num, option_num), test: (episode_num, timestep_num, option_num)
        "all_values": {"train": [], "test": []},  # train: (episode_num, optim_epoch, timestep_num, option_num), test: (episode_num, timestep_num, option_num)
    })
	...
	agent_OC_m4.train(**_train_kwargs)
	...
	plot_OC_m3.plot_advantage(rec.advantages, "advantage", args.ckpt_dir)
	plot_OC_m3.plot_advantage(rec.beta_advantages, "advantage_beta", args.ckpt_dir)
	...
	for name in ["train", "test"]:
		option_indices = rec.option_indices[name] 
		if name == "train":
			frequency = 1
			option_betas = [np.array(betas).mean(axis=0).tolist() for betas in rec.all_betas[name]]
			option_values = [np.array(values).mean(axis=0).tolist() for values in rec.all_values[name]]
		else:
			frequency = args.test_frequency
			option_betas = rec.all_betas[name]
			option_values = rec.all_values[name]
		plot_OC_m4.plot_option_duration(option_indices, args.option_num, frequency, name, args.ckpt_dir)
		plot_OC_m4.plot_option_beta(option_betas, option_indices, args.option_num, frequency, name, args.ckpt_dir)
		if args.option_num > 1:
			plot_OC_m4.plot_option_margin(option_values, frequency, name, args.ckpt_dir)
